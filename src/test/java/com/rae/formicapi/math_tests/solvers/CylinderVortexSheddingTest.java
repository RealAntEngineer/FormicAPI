 ...
 */
public class CylinderVortexSheddingTest {
    ...
    //TODO the negative production with negative vorticity could be a big issue ->
    // maybe use the square of it for the production ? or just be on always on laminar flow for the game
    //@Test
    void vortexSheddingBehindCylinder() {
        ...
        double muInf = 3;                     // standard SA freestream value ~3*nu
        ...
        buildCylinderOperator(nx, ny, hx, hy, nu, cylinderCenterX, cylinderCenterY, R, uInf, muInf,
                cb1, cw1, sigma, cv, pressurePenalty, 1.0 / dt, C, A, scaling, role, bcValue, wallDistance);
        ...
        for (int step = 0; step < nSteps; step++) {
            ...
            if (step % printEvery == 0 || step == nSteps - 1) {
                ...
                Field2DRenderer.saveHeatmap(speed, String.format("cylinder_shedding/speed_%05d.png", step),
                        Field2DRenderer.Interpolation.NEAREST);
                Field2DRenderer.saveHeatmap(vorticity, String.format("cylinder_shedding/vorticity_%05d.png", step),
                        Field2DRenderer.Interpolation.NEAREST);
            }
        }
    }
