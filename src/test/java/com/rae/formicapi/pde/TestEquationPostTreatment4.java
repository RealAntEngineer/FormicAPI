...
public class TestEquationPostTreatment4 {
    ...
    void sameAxisSecondDerivativeCollapses() {
        ...
        assertEquals("d2dx2(T)", ScalarAlgebra.distribute(expr).toString());
    }

    @Test
    void yAxisSecondDerivativeCollapses() {
        Expression expr = parse("ddy(ddy(T))");
        assertEquals("d2dy2(T)", ScalarAlgebra.distribute(expr).toString());
    }

    @Test
    void zAxisSecondDerivativeCollapses() {
        Expression expr = parse("ddz(ddz(T))");
        assertEquals("d2dz2(T)", ScalarAlgebra.distribute(expr).toString());
    }

    @Test
    void mixedPartialXyCollapses() {
        Expression expr = parse("ddx(ddy(T))");
        assertEquals("d2dxdy(T)", ScalarAlgebra.distribute(expr).toString());
    }
    ...
    void mixedPartialXzCollapses() {
        ...
        assertEquals("d2dxdz(T)", ScalarAlgebra.distribute(expr).toString());
    }

    @Test
    void mixedPartialYzCollapses() {
        Expression expr = parse("ddz(ddy(T))");
        assertEquals("d2dydz(T)", ScalarAlgebra.distribute(expr).toString());
    }
    ...
    void ddtDoesNotCollapseWithAxisDerivative() {
        ...
        assertEquals("ddx(ddt(T))", ScalarAlgebra.distribute(expr).toString());
    }
    ...
    void axisSecondDerivativeCollapsesAfterLinearityDistribution() {
        ...
        assertEquals("(d2dx2(T) + d2dx2(T))", ScalarAlgebra.distribute(expr).toString());
    }
}
