...
public class TestEquationPostTreatment2 {
    ...
    void combineIdenticalTerms() {
        ...
        assertEquals("(2.0 * a)", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }

    @Test
    void combineThreeIdenticalTerms() {
        Expression expr = parse("a+a+a");
        assertEquals("(3.0 * a)", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }

    @Test
    void cancellingTermsBecomeZero() {
        Expression expr = parse("a-a");
        assertEquals("0.0", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }

    @Test
    void combineLikeTermsLeavesDistinctTermsAlone() {
        Expression expr = parse("a+b");
        assertEquals("(a + b)", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }

    @Test
    void combineLikeTermsRecursesIntoMultiplication() {
        Expression expr = parse("a*(b+b)");
        assertEquals("(a * (2.0 * b))", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }

    @Test
    void combineLikeTermsRecursesIntoUnaryOperator() {
        Expression expr = parse("grad(a+a)");
        assertEquals("grad((2.0 * a))", ExpressionAlgebra.combineLikeTerms(expr).toString());
    }
    ...
    void factorSimpleCommonFactor() {
        ...
        assertEquals("(a * (c + 1.0))", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }

    @Test
    void factorReducesToCombineWhenTermsIdentical() {
        Expression expr = parse("a+a");
        assertEquals("(2.0 * a)", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }

    @Test
    void factorThreeTermsWithSharedFactor() {
        Expression expr = parse("a*c+a*d+a");
        assertEquals("(a * ((c + d) + 1.0))", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }

    @Test
    void noCommonFactorLeavesExpressionAlone() {
        Expression expr = parse("a+b");
        assertEquals("(a + b)", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }
    ...
    void factorRecursesIntoUnaryOperator() {
        ...
        assertEquals("grad((a * (c + 1.0)))", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }

    @Test
    void factorCommonFactorRecursesIntoMultiplication() {
        Expression expr = parse("b*(a*c+a)");
        assertEquals("(b * (a * (c + 1.0)))", ExpressionAlgebra.factorCommonFactor(expr).toString());
    }
}