 ...
 * force accumulator, and mass as parallel {@link RealVector}s (one per
 ...
 * builds a {@code Matrix} against a set of {@link RealVector}s.
 */
public final class ParticleSystem {

    private final int dim;

    private final RealVector[] position;   // dim vectors
    private final RealVector[] velocity;   // dim vectors
    private final RealVector[] force;      // dim vectors, accumulator
    private final RealVector   mass;

    private final BooleanVector alive;

    private int count;   // active slots occupy [0, count)

    public ParticleSystem(int dim, Supplier<RealVector> doubleFactory, Supplier<BooleanVector> boolFactory) {
        this.dim = dim;
        position = new RealVector[dim];
        velocity = new RealVector[dim];
        force    = new RealVector[dim];
        for (int d = 0; d < dim; d++) {
    ...
    public RealVector position(int axis) { return position[axis]; }
    public RealVector velocity(int axis) { return velocity[axis]; }
    public RealVector force(int axis)    { return force[axis]; }
    public RealVector mass()             { return mass; }

    public void clearForces() { for (RealVector f : force) f.clear(); }
     ...
     * indices; then one batched {@link RealVector#gather} per field
